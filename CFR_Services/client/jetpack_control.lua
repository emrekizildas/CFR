		...
		if( not isPedOnGround( _local ))  then
			if( currentFuel > 0 ) then
				...
				if( currentFuel == 0 ) then
					...
					outputChatBox( "Jetpack'inin benzini bitti!", 255, 255, 0 )
				end
...
function onClientResourceStart(res)
	if res == getThisResource() then
		jMenu = guiCreateWindow( x - 130, y - 210, 120, 120, "Jetpack      ", false ) -- Titles are NOT in the center
		guiWindowSetMovable( jMenu, false )
		guiWindowSetSizable( jMenu, false )
		
		jImage = guiCreateStaticImage( x - 102, y - 195, 64, 64, "images/ammu/JETPACK.png", false, nil )
		jBar = guiCreateProgressBar( x - 125, y - 128, 100, 10, false, nil )
		jText = guiCreateLabel( x - 125, y - 115, 100, 15, "", false )
...
function onClientResourceStop(res)
	if res == getThisResource() then
		guiSetVisible( jMenu, false )
