...
function requestRespray() 
	if( getPlayerMoney( source ) >= paynsprayCosts ) then
		...
		setTimer( function (s,v) 
			...
			outputChatBox( "Your Vehicle has been resprayed! Costs: $" .. paynsprayCosts, s, 0, 255, 0 )
			triggerEvent( "onPayNSprayFix", getRootElement(), v, s )
			
			end, 1000, 1, source, vehicle )
	else
		outputChatBox( "You don't have enough cash to pay me, the price is $" .. paynsprayCosts, source, 255, 255, 0 )
	end
end
